...
{
    for (int y = blockIdx.y * blockDim.y + threadIdx.y + 1; y < HALF_HEIGHT; y += blockDim.y * gridDim.y) {

        const double ci = y * STEP + MIN_Y;

        for (int x = blockIdx.x * blockDim.x + threadIdx.x + 1; x < WIDTH; x += blockDim.x * gridDim.x) {
